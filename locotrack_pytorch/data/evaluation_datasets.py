...
class RoboticsStackingDataset(Dataset):
    def __init__(
        self,
        robotics_points_path: str,
        query_mode: str = 'strided',
        ...
        """
        :param robotics_points_path: Path to the pickle file.
        :param query_mode: 'strided' or 'first'.
        :param resolution: (height, width) tuple for resizing, or None to skip resizing.
        """
        super().__init__()
        # Load entire dataset once in constructor. If the file is huge,
        # consider using a more suitable data format or chunked approach.
        with open(robotics_points_path, 'rb') as f:
            self.robotics_points_dataset = pickle.load(f)
        
        self.query_mode = query_mode
        self.resolution = resolution

    def __len__(self):
        return len(self.robotics_points_dataset)

    def __getitem__(self, idx):
        example = self.robotics_points_dataset[idx]
        frames = example['video']
        ...
        # Scale target points according to image width/height
        target_points = example['points']
        target_occ = example['occluded']
        ...
        # If you don't need the `[0]` index, you can remove that part;
        # it depends on how your sample_queries_* functions structure their output.
        data = {}
        for k, v in converted.items():
            if isinstance(v, np.ndarray):
                # If v has shape (1, ...) or you only need the first element, keep [0].
                # Otherwise, you might just do `torch.tensor(v)`.
...
class KineticsDataset(Dataset):
    ...
    def __init__(
        ...
        from glob import glob
        self.all_paths = sorted(glob(path.join(kinetics_path, '*_of_0010.pkl')))
...
def get_eval_dataset(mode, path, resolution=(256, 256)):
    ...
    if 'robotics' in mode:
        key = 'robotics'
        datasets[key] = RoboticsStackingDataset(
            path[key], query_mode, resolution
        )
    if 'kinetics' in mode:
        key = 'kinetics'
        datasets[key] = KineticsDataset(
            path[key], query_mode, resolution
        )
    if 'robotap' in mode:
        key = 'robotap'

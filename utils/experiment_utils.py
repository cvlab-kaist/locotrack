...
from locotrack import optimizers
...
class NumpyFileCheckpointer(utils.Checkpointer):
  ...
  def save(self, ckpt_series: str) -> None:
    ...
    with tf.io.gfile.GFile(self._checkpoint_file + f'_iter{global_step:0>7}', 'wb') as fp:
      np.save(fp, to_save)

    with tf.io.gfile.GFile(self._checkpoint_file + f'_tmp', 'wb') as fp:
      np.save(fp, to_save)
    tf.io.gfile.rename(

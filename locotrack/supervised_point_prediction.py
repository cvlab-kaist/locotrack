...
class SupervisedPointPrediction(task.Task):
  ...
  def _build_eval_input(
    ...
    elif 'eval_robotap_points' in mode:
      yield from evaluation_datasets.create_robotap_dataset(
          self.config.robotap_points_path,
          query_mode=query_mode,
          resolution=self.eval_inference_resolution,
      )
  ...
  def _eval_epoch(
    ...
    for inputs in self._build_eval_input(mode):
      ...
      # import os
      # os.makedirs(f'scores/{mode}', exist_ok=True)
      # if os.path.exists(f'scores/{mode}/eval_{batch_id + 1:0>5}.npy'):
      #   batch_id += 1
      #   continue
      ...
      # np.save(f'scores/{mode}/eval_{batch_id:0>5}.npy', scalars)

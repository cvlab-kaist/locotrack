...
from locotrack.data.kubric_data import create_point_tracking_dataset
from locotrack import supervised_point_prediction
from locotrack import locotrack_model
from locotrack import task
from locotrack.utils import experiment_utils as exputils
...
class Experiment(experiment.AbstractExperiment):
  ...
  def __init__(
    ...
    def forward(*args, is_training=True, **kwargs):
      ...
      return self.point_prediction.forward_fn(
          *args,
          ...
          refinement_resolutions=self.config.refinement_resolutions,
          **kwargs,
      )
    ...
    wandb.init(
      ...
      id=os.path.basename(self.config.checkpoint_dir),
      resume='allow',
    )
  ...
  def _construct_shared_modules(self) -> Mapping[str, task.SharedModule]:
    ...
    shared_module_constructors = {
        'locotrack_model': locotrack_model.LocoTrack,
    }
    shared_modules = {}
  ...
  def step(  # pytype: disable=signature-mismatch  # numpy-scalars
    ...
    if utils.get_first(global_step) % 10 == 0:
      wandb.log(scalars, step=utils.get_first(global_step))
    ...
    if ((utils.get_first(global_step) + 1) % self.config.evaluate_every) == 0:
      for mode in self.config.eval_modes:
        ...
        wandb.log(eval_scalars, step=utils.get_first(global_step))
  ...
  def _initialize_train(self):
    ...
    init_params, self._state = init_net(init_rng, inputs)

    if self._params is not None:
      pretrained_params = {k: v for k, v in self._params.items() if 'resnet' in k or 'cost_conv' in k or 'occ_linear' in k} # only load resnet params
      self._params = {**init_params, **pretrained_params}
    else:
  ...
  def _build_train_input(
    ...
    dataset_constructors = {
        'kubric': create_point_tracking_dataset,
    }
    dataset_generators = {}
  ...
  def _update_func(
    ...
    # exclude resnet weights
    # updates = jax.tree_util.tree_map_with_path(
    #   lambda path, x: x if 'resnet' not in path[0].key else jnp.zeros_like(x), 
    #   updates
    # )
